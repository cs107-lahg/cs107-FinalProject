...
def test_arcsin():
    ...
    x = ad.Variable(
        np.array([value1, value2]), np.array([deriv_seed1, deriv_seed2])
    ).arcsin()
    ...
    assert np.array_equal(
        ...
        np.array(
            [
                1 / (np.sqrt(1 - value1 ** 2)) * deriv_seed1,
                1 / (np.sqrt(1 - value2 ** 2)) * deriv_seed2,
            ]
        ),
    )
...
def test_arctan():
    ...
    assert np.array_equal(
        x.der,
        np.array(
            [1 / (1 + value1 ** 2) * deriv_seed1, 1 / (1 + value2 ** 2) * deriv_seed2]
        ),
    )
...
def test_tanh():
    ...
    assert np.array_equal(
        x.der, np.array([(1 / (np.cosh(1) ** 2)) * 3, (1 / (np.cosh(2) ** 2)) * 4])
    )
...
def test_ne():
    ...
    assert np.not_equal(
        ad.Variable(np.array([1, 2]), np.array([1, 2])),
...
def test_add():
    ...
    assert np.array_equal(
        (ad.Variable(np.array([1, 2]), np.array([1, 2])) + 1).val, np.array([2, 3])
    )
    assert np.array_equal(
        (ad.Variable(np.array([1, 2]), np.array([1, 2])) + 2).der, np.array([1, 2])
    )
    assert np.array_equal(
        (1 + ad.Variable(np.array([1, 2]), np.array([1, 2]))).val, np.array([2, 3])
    )
    assert np.array_equal(
        (1 + ad.Variable(np.array([1, 2]), np.array([1, 2]))).der, np.array([1, 2])
    )
...
def test_sub():
    ...
    assert np.array_equal(
        (ad.Variable(np.array([1, 2]), np.array([1, 2])) - 1).val, np.array([0, 1])
    )
    assert np.array_equal(
        (ad.Variable(np.array([1, 2]), np.array([1, 2])) - 2).der, np.array([1, 2])
    )
...
def test_rsub():
    ...
    assert np.array_equal(
        (1 - ad.Variable(np.array([1, 2]), np.array([1, 2]))).val, np.array([0, -1])
    )
    assert np.array_equal(
        (1 - ad.Variable(np.array([1, 2]), np.array([1, 2]))).der, np.array([-1, -2])
    )
...
def test_mul():
    ...
    x = ad.Variable(np.array([1, 2]), np.array([3, 4])) * ad.Variable(
...
def test_rmul():
    ...
    x = ad.Variable(np.array([2, 2]), np.array([2, 2])) * ad.Variable(
...
def test_pow():
    ...
    assert np.array_equal(
        x.der, np.array([-3.5 * 5 ** (-3.5 - 1) * 5, -3.5 * 2 ** (-3.5 - 1) * 6])
    )
    ...
    assert np.array_equal(
        ...
        np.array(
            [
                2 * 3 ** (2 - 1) * 5 + np.log(3) * 3 ** 2 * 1,
                1 * 2 ** (1 - 1) * 6 + np.log(2) * 2 ** 1 * 2,
            ]
        ),
    )
    ...
    assert np.array_equal(
        x.der, np.array([np.log(2) * 2 ** 0 * 5, np.log(2) * 2 ** 2 * 6])
    )
...
def test_log():
    ...
    x = ad.Variable(np.array([1, 2]), np.array([3, 4])).log()
    assert np.array_equal(
        x.val, np.array([np.log(1) / np.log(10), np.log(2) / np.log(10)])
    )
    assert np.array_equal(np.round(x.der, 8), np.array([1.30288345, 0.86858896]))
...
def test_sqrt():
    ...
    assert ad.Variable(1).sqrt().der == 0.5
    with pytest.raises(ValueError):
        ad.Variable(-1).sqrt()
    with pytest.raises(ValueError):
        ad.Variable(0).sqrt()
...
def test_make_variables():
    ...
    v = ad.make_variables(
        [np.array([3, 4]), np.array([1, 5])], [np.array([1, 2]), np.array([1, 5])]
    )
...
def test_logistic():
    ...
    assert np.array_equal(
        np.round(f.val, 8), np.array([0.73105858, 0.88079708, 0.95257413])
    )
    assert np.array_equal(
        np.round(f.der, 8), np.array([0.19661193, 0.10499359, 0.04517666])
    )
