...
| self.logistic | This method performs the logistic operation. The new value is calculated using 1 / (1 + np.exp(-self.val)) and the new derivative is calculated using np.exp(-self.val) / ((1 + np.exp(-self.val)) \*_ 2) _ self.der. A new Variable object is returned with the new value and derivative. |
